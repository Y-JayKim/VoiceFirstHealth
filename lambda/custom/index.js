...
const newSessionHandlers = {
    LaunchRequest() {
        this.handler.state = 'NAME_MODE';
        this.emit(":ask", 'Welcome, what is your name?');
    }
};

const nameHandler = Alexa.CreateStateHandler("NAME_MODE", {
    'UserNameIntent': function () {
        // handleUserGuess.call(this, false);
        const inputUserName = this.event.request.intent.slots.Username.value;
        let speechOutput = '';
    
        speechOutput = "Hello " + inputUserName + ' what topic interests you today?';
...
const topicStateHandlers = Alexa.CreateStateHandler(GAME_STATES.TOPIC, {
    'TopicIntent': function () {
        ...
        if(userTopic === "concussion"){
            ...
            this.emit(":ask", 'Which concussion module would you like to access today?');
        } else {
            this.handler.state = GAME_STATES.TOPIC;
...
const concussionTopicHandlers = Alexa.CreateStateHandler("CONCUSSION_MODE", {
    ...
    'Unhandled': function () {
        this.handler.state = "CONCUSSION_MODE";
        this.emit(":ask", "That is not currently available, please ask for an available module.  For example, ask for assessment. ");
...
exports.handler = function (event, context) {
    ...
    alexa.registerHandlers(newSessionHandlers, nameHandler, topicStateHandlers, concussionTopicHandlers, assessStateHandlers, startStateHandlers, triviaStateHandlers, helpStateHandlers);
    alexa.execute();
};